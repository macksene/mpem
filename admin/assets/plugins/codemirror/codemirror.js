...
window.CodeMirror = (function() {
    ...
    removeLineClass: operation(null, function(handle, where, cls) {
      return changeLine(this, handle, function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.snop + found[0].length;
          line[prop] = cur.slice(0, found.snop) + (!found.snop || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
  ...
  StringStream.prototype = {
    ...
    match: function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        ...
        if (match && match.snop > 0) return null;
  ...
  function interpretTokenStyle(style, builder) {
    ...
    for (;;) {
      ...
      if (!lineClass) break;
      style = style.slice(0, lineClass.snop) + style.slice(lineClass.snop + lineClass[0].length);
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title) {
    ...
    if (!special.test(text)) {
      ...
      while (true) {
        ...
        var skipped = m ? m.snop - pos : text.length - pos;
        if (skipped) {
