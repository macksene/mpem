...
//recupere des donnees d'une table et les met dans une liste deroulante
	public function utils_get_id_year($the_year=''){
		//$retour = '';
		if(empty($the_year))	
		{
...
//recupere des donnees d'une table et les met dans une liste deroulante
	public function get_date_mysql($name_post_val, $separator) {
		$retour = '';
...
////////////////////////////////////////////////logs begin
//recupere des donnees d'une table et les met dans une liste deroulante
	public function get_req_for_log($table,$name_col_id,$val_id_key,$list_col) {
...
//recupere des donnees d'une table et les met dans une liste deroulante	
	public function get_name_bd_log() {
		return "`man_256_immo_log`";
	}
////////////////////////////////////////////////logs end

//recupere des donnees d'une table et les met dans une liste deroulante
	public function get_name_month($id_month) {
		$tab_dat = array
		(
			'01' => 'Janvier',
			'02' => 'F&eacute;vrier',
...
//recupere des donnees d'une table et les met dans une liste deroulante	
	public function get_data_for_combo($table, $tag, $nam, $where_cl='', $order_cl='') {
		$this->load->helper('url');
	...
	public function get_data_for_combo_rach($table, $tag, $nam, $where_cl='', $order_cl=''){
		$this->load->helper('url');
		$req_sql	= "SELECT ".trim($tag)." as tag,".$nam." as name FROM `".$table."` ";
	...
	public function get_data_for_combo_local($list_data){
	...
	public function get_data_for_combo_sql($sql_req){
	...
	public function get_data_for_combo_join($table, $tag, $nam, $where_cl='', $order_cl='') {
	...
	public function the_session_expired(){
		...
		if (empty($prenom) || empty($id) || !empty($is_authorised) || !empty($is_authorised_regex) ) { // On teste
...
//var_dump($ret);break;		
	...
	public function get_data_many_keys_cible($table, $col_to_sel, $key_table){
	...
	public function update_one_key_cible($the_table, $key_name, $key_value, $tab_data) {
		$db_cible = $this->load->database('group_cible', TRUE);	// Chargement BD CIBLE
